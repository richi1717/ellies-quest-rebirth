...
import reducer from '../../src/reducers/reducer_get-menu-items-selected';
import { fromJS } from 'immutable';

describe('menu items selected reducer', () => {
  it('should return the initial state', () => {
    expect(reducer(undefined, {})).to.equal(fromJS([false]));
  });
  it('handles SET_MENU_ITEMS_SELECTED', () => {
    const DEF = fromJS([false]);
    const action = { type: types.SET_MENU_ITEMS_SELECTED, payload: true };
